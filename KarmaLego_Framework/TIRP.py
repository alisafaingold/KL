...
class TIRP (object):
    ...
    def hollow_copy(self, with_label=False):
        """
        create new tirp and copy all current variables
        :return: TIRP,copy of this tirp
        """
        ...
        new_tirp.mean_offsets = copy(self.mean_offsets)
        if with_label:
            new_tirp._supporting_sequences_by_entity = self._supporting_sequences_by_entity
        return new_tirp
    ...
    def save_tirp(self, path, num_relations, label='0', with_label=False):
        """
        ...
        """
        t = self.hollow_copy(with_label=with_label)
        # file_name = f"{self.get_tirp_file_name(num_relations)} class-{label}"
        file_name = f"{self.get_tirp_file_name(num_relations)}"
        if with_label:
            file_name = f'{file_name} class_{label}'
        print(f'{path}/{file_name}')

        with open(f'{path}/{file_name}.pickle', 'wb') as handle:
            pickle.dump(t, handle, protocol=pickle.HIGHEST_PROTOCOL)

    def print_tirp(self, path, entities_vector, num_relations, extended_output=True,
        ...
        """
        print(path)
        fila_name = self.get_tirp_name(num_relations)
        print({path}/{fila_name})
        with open(f'{path}/{fila_name}.pickle', 'wb') as handle:
