...
class TIRPsFeatureExraction_with_stinstance (object):
    def __init__(self, min_ver_support=0.5):
        self._min_ver_support = min_ver_support

    def create_feature_matrix(self, tirps, entities_list, representation, entities_class, entities_class_header='CLASS',
                              entities_list_header='ID'):
        """
    ...
    @staticmethod
    def getFeaturesListByTIRP(tirp, entities_list, representation):
        """
        ...
        if representation == 'BIN':
            feature_list = [1 if (x in tirp.supporting_entities) else 0 for x in entities_list]
        elif representation == 'HS' or representation == 'HSN':
            for x in entities_list:
                if x in tirp.supporting_entities:
                    feature_list.append(len([instance.sti for instance in tirp.supporting_instances if
                                             instance.entity_id == x]))
                else:
                    feature_list.append(0)
        elif representation == 'MD':
            for entity in entities_list:
                instances = [instance.sti for instance in tirp.supporting_instances if instance.entity_id == entity]
                if len(instances) == 0:
                    feature_list.append(0)
                else:
    ...
    def getMatrixForModeling(self, tirps, entities_list, representation, num_of_relations, label):
        ...
        :param label: 0/1
        :param num_of_relations: int, relations number (7 for example)
        :param entities_list: the entities list in the population
    ...
    @staticmethod
    def concat_matrix_classes(matrixes):
    ...
    def HSN(self, entities_list, tirps, data, num_of_relations):
        for entity_index in range(0, len(entities_list)):
            _max = 1
            for tirp in tirps:
                if data[tirp.get_tirp_name(num_of_relations)][entity_index] > _max:
                    _max = data[tirp.get_tirp_name(num_of_relations)][entity_index]
            for tirp in tirps:
                data.loc[entity_index, tirp.get_tirp_name(num_of_relations)] = \
                    float(data.loc[entity_index, tirp.get_tirp_name(num_of_relations)] / _max)
        return data
