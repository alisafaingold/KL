...
class Lego(object):
            ...
            for symbol2_id, symbol2 in self._karma.get_symbols_map().items():
                for rel in range(self._karma._num_relations):
                    ...
                    if processes_num:
                        ...
                        print('enter to processes_num')
                        pool.apply_async(self.extend_tirp, (tirp, index_same, skip_followers))
                        # )
                    else:
                        root_node.add_node(self.extend_tirp(tirp, index_same, skip_followers))
        if processes_num:
            # c_size = int(len(nodes_processes)/(processes_num*3))
            # pool.starmap(self.extend_tirp, nodes_processes, chunksize=c_size)
            pool.close()
            pool.join()
        # self.insert_frequent_tirps(root_node)
        print('Lego fit complete')

    def insert_frequent_tirps(self, node):
        if node.tirp:
            self.frequent_tirps.append(node.tirp)
        for n in node.nodes:
            self.insert_frequent_tirps(n)
    ...
    def extend_tirp(self, tirp, index_same, skip_followers):
        """
         ...
         :return: None
        """
        print(tirp.get_tirp_name(self._karma._num_relations))
        # root_node = LegoTreeNode(tirp)
        # self.frequent_tirps.append(tirp) #T
                ...
                if (self._karma.get_vertical_support_of_sym_to_sym_rel(
                    ...
                    for candidate_relations in candidates:
                        ...
                        if len(new_tirp.supporting_entities) >= self._karma.get_min_vertical_support():
                            ...
                            if self.incremental_output:
                                tirp.save_tirp(f'{self.path}', self._karma._num_relations, self.label)

                            if new_tirp.size < self._max_tirp_length:
                               self.extend_tirp(new_tirp, index_same, skip_followers)

        # tirp_file_name = tirp.get_tirp_file_name(self._karma._num_relations)
        tirp.save_tirp(f'{self.path}', self._karma._num_relations, self.label, with_label=True)
        return None
    ...
    def print_frequent_tirps(self, path):
        ...
        for tirp, i in zip(self.frequent_tirps, range(len(self.frequent_tirps))):
            ...
            #     continue
            tirp.print_tirp(path + tirp_file_name,
                            self._karma.get_entities_vector(), self._karma._num_relations, Karma.CALC_OFFSETS,
